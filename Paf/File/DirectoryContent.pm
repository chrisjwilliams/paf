# ----------------------------------
# class Paf::File::DirectoryContent
# Description:
#   Object to access the contents of a directory
#-----------------------------------
# Methods:
# new(dirname , depth=-1) : depth=-1 => unlimited
...
package Paf::File::DirectoryContent;
...
sub new {
    ...
    $self->{dir}=shift;
    $self->{depth}=shift;
    if(! defined $self->{depth}) { $self->{depth}=-1; }
    $self->exclude(@_);
    $self->{parsed}=0;
    return $self;
}
