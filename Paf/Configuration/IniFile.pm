...
# readFile(filename) :  data from the specified ini file
...
package Paf::Configuration::IniFile;
use strict;
use Carp;
1;
...
sub setList {
    ...
    foreach my $line ( @_ ) {
        ...
        #    $self->{lists}{$section}{$1}=0;
             ...
             push @{$self->{content}{$section}}, $1;
        }
        else {
             push @{$self->{listArray}{$section}}, $line;
             push @{$self->{content}{$section}}, $line;
...
sub readStream {
    ...
    while ( <$fh> ) {
        ...
        # - keep a record of unprocessed section content
        push @{$self->{content}{$section}}, $line;
